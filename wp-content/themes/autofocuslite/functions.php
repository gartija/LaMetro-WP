...
if ( !function_exists( 'optionsframework_init' ) ) {
	...
	if ( STYLESHEETPATH == TEMPLATEPATH ) { 
		...
		define('OPTIONS_FRAMEWORK_DIRECTORY', STYLESHEET_DIR . '/inc/options/');
	} else {
		define('OPTIONS_FRAMEWORK_PATH', TEMPLATEPATH . '/inc/options/');
		define('OPTIONS_FRAMEWORK_DIRECTORY', TEMPLATE_DIR . '/inc/options/');
	}
...
function autofocus_nav_above() {
	...
	$autofocus_blog_catid = of_get_option($shortname . '_blog_cat');

	if ( in_category($autofocus_blog_catid)) : ?>
	...
	<?php else : ?>
				<nav id="nav-above" class="navigation">
					<div class="nav-previous"><?php previous_post_link('%link', __('<span class="meta-nav">&larr;</span>', 'autofocus'), 0, $autofocus_blog_catid) ?></div>
					<div class="nav-next"><?php next_post_link('%link', __('<span class="meta-nav">&rarr;</span>', 'autofocus'), 0, $autofocus_blog_catid) ?></div>
				</nav><!-- #nav-above -->
	<?php endif; 
}
...
function autofocus_nav_below() {
	...
	<?php else : ?>
			...
			<nav id="nav-below" class="navigation">
				<h3><?php _e('Browse', 'autofocus') ?></h3>
			<?php 
				$previouspost = get_previous_post(FALSE, $autofocus_blog_catid);
				if ($previouspost != null) { 
					echo '<div class="nav-previous">';
					previous_post_link('<span class="meta-nav">&larr;</span> Older: %link', '%title', FALSE, $autofocus_blog_catid);
					echo '<div class="nav-excerpt">';
					previous_post_excerpt(FALSE, $autofocus_blog_catid);
					echo '</div></div>';
				 } ?>
	
			<?php 
				$nextpost = get_next_post(FALSE, $autofocus_blog_catid);
				if ($nextpost != null) {
					echo '<div class="nav-next">';
					next_post_link('Newer: %link <span class="meta-nav">&rarr;</span>', '%title', FALSE, $autofocus_blog_catid);
					echo '<div class="nav-excerpt">';
					next_post_excerpt(FALSE, $autofocus_blog_catid);
					echo '</div></div>';
				 } ?>
...
if ( is_admin() && isset($_GET['activated'] ) && $pagenow == 'themes.php' ) {
	...
	update_option( 'date_format', __('j M &#8217;y', 'autofocus') );
}
...
function autofocus_layout_class($class = '') {
	...
	$autofocus_classes = array();
	
	// Which layout is being used?
	if ( is_archive() || is_search() )
		$autofocus_classes[] = 'normal-layout';
	
	if ( is_home() )
		$autofocus_classes[] = 'af-default';

	// Output classes
	$class_str = implode( ' ', $autofocus_classes );
	echo $class_str;
